...
export function HelpBottomSheet({ isOpen, onClose }: HelpBottomSheetProps) {
  ...
  return (
    <>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 mx-auto w-full max-w-lg rounded-t-2xl bg-bg-primary shadow-2xl" style={{ maxHeight: '50dvh', animation: 'slide-up 0.25s ease-out' }}>
        <div className="flex items-center justify-center pt-2 pb-1">
          <div className="h-1 w-10 rounded-full bg-gray-300" />
        </div>
        <div className="px-5 pb-2">
          <h2 className="text-lg font-bold text-text-primary">Album Review Guide</h2>
        </div>
          ...
          <ul className="space-y-4">
            {helpItems.map((item, i) => (
              <li key={i} className="flex items-start gap-3">
                ...
                <p className="text-base leading-relaxed text-text-secondary pt-0.5">{item}</p>
              </li>
            ))}
          </ul>
          <button
