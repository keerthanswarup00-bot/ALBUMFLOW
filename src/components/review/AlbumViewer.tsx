...
const FlipPage = forwardRef<HTMLDivElement, { page: ReviewPage }>(({ page }, ref) => {
  return (
...
export function AlbumViewer({ album, pages }: AlbumViewerProps) {
  ...
  const flipWidth = firstPage ? Math.round(firstPage.width / 5) : 400;
  const flipHeight = firstPage ? Math.round(firstPage.height / 5) : 560;
  ...
  const handleOrientationChange = useCallback((e: { data?: string; object?: { getOrientation?: () => string } }) => {
    const mode = (e.data || e?.object?.getOrientation?.() || 'portrait') as 'portrait' | 'landscape';
    setFlipMode(mode);
  }, []);

  const handleInit = useCallback((e: { page?: number; mode?: string }) => {
    const pageIndex = typeof e.page === 'number' ? e.page : 0;
    const mode = (e.mode || 'portrait') as 'portrait' | 'landscape';
    setCurrentPage(pageIndex);
    setFlipMode(mode);
  }, []);
